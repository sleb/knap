...
| 6 — `knap rename-heading`                             | Done   |                                                                                                                                                                                                                                                                                                       |
| 7 — `knap rename-tag`                                 | Done   | `config::for_path` stores whatever root it's given verbatim as `index_roots`; a relative `"."` produced relative `note.path`s that panicked in `path_to_uri`. Fixed by absolutizing `.` before indexing, same pattern `run_file` already used.                                                        |
| 8 — Integration tests + docs                          | Done   | `run_file`/`run_heading` were scoping the index off the target file's own path — `config::for_path` treats a file argument's parent directory as the whole root, silently dropping notes elsewhere in the vault from the index. Fixed both to scope off `cwd`, matching `run_tag`'s existing pattern. |
