...
pub fn run(
    ...
    let (mut idx, _) = index::build(&config.index_roots, &config.path_filter)?;

    let mut fixes_applied = None;
    if fix {
        let abs_path = absolute(path)?;
        let fix_config = config::for_path(&abs_path, None, exclude)?;
        let (fix_idx, _) = index::build(&fix_config.index_roots, &fix_config.path_filter)?;
        ...
        idx = index::build(&config.index_roots, &config.path_filter)?.0;
    }
